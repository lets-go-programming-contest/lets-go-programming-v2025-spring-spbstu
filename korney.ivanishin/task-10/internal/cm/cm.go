...
var (
        ...
        ErrNameRequired     = errors.New("non-empty name required")
...
func NewContact(name string, number string) Contact {
        return Contact{
                ID:     uuid.New().String(),
                Name:   name,
                Number: number,
...
func (contMan *ContMan) Add(contact Contact) error {
        if contact.Name == `` {
                return errors.Join(errFailedAddCont, ErrNameRequired)
        }
